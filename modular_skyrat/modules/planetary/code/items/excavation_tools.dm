...
/obj/item/excavation_pick
	...
	icon = 'modular_skyrat/modules/planetary/icons/items/excavation_tools.dmi'
	icon_state = "pick1"
...
/obj/item/excavation_locator/proc/do_scan(mob/user)
	...
	for(var/turf/MT in view(T, world.view))
		if(MT.GetComponent(/datum/component/turf_excavation))
			minerals += MT
	if(length(minerals))
		for(var/i in minerals)
			var/turf/M = i
			var/obj/effect/temp_visual/strange_rock_overlay/oldC = locate(/obj/effect/temp_visual/strange_rock_overlay) in M
			if(oldC)
				qdel(oldC)
