...
/datum/component/turf_excavation/Initialize(amount)
	. = ..()
	if(!isturf(parent))
...
/datum/component/turf_excavation/proc/DoDig(mob/user, dug_depth)
	current_dig_depth += dug_depth
	var/atom/A = parent
	if(!rocky_overlay)
		rocky_overlay = mutable_appearance('modular_skyrat/modules/planetary/icons/effects/excavation.dmi', "rock")
	...
	for(var/text_depth in find_list)
		...
		if(current_dig_depth > actual_depth) //We broke it
			...
			if(current_dig_depth == actual_depth) //perfect dig
				our_artifact.forceMove(A)
			else
...
/datum/component/turf_excavation/Destroy(force, silent)
	var/atom/A = parent
	for(var/dpt in find_list)
		...
		qdel(iterated_find)
	if(rocky_overlay)
		A.cut_overlay(rocky_overlay)
		qdel(rocky_overlay)
	return ..()

/datum/component/turf_excavation/RegisterWithParent()
	RegisterSignal(parent, COMSIG_PARENT_ATTACKBY, .proc/AttackItem)

/datum/component/turf_excavation/UnregisterFromParent()
	UnregisterSignal(parent, COMSIG_PARENT_ATTACKBY)

/datum/component/turf_excavation/proc/AttackItem(datum/source, obj/item/I, mob/living/user)
	var/atom/A = parent
